...
opt.no_flip = True
opt.no_instance = bool(carla_settings['general']['no_instance'])
...
scale_instance = bool(carla_settings['general']['scale_instance'])
use_label_as_instance = bool(carla_settings['general']['use_label_as_instance'])
map_instances = bool(carla_settings['general']['map_instances'])
cityscapes_label = list(carla_settings['general']['cityscapes_label'])
cityscapes_instance = list(carla_settings['general']['cityscapes_instance'])
...
def preprocess(label,inst_label):
    label = label.astype(np.uint8)
    inst_label = inst_label.astype(np.uint8)

    A = Image.fromarray(label)
    ...
    inst = Image.fromarray(inst_label)
    inst_tensor = transform_A(inst)

    if scale_instance:
        inst_tensor = inst_tensor * 255.0

    input_dict = {'label': A_tensor, 'inst': inst_tensor, 'image': torch.tensor([0]),
                  'feat': torch.tensor([0]), 'path': "null"}
...
try:
    ...
    while not done:
        ...
        inst_label = sensor_data['label_inst'][:,:,1]
        ...
        #you may want to add instance mapping based on Cityscapes instances for vehicles and pederestrians.
        if map_instances:
            old_class_indexes = cityscapes_label #cityscapes label
            new_integer_ids = cityscapes_instance #corresponding instance label
            inst_label = np.select([label == old_idx for old_idx in old_class_indexes], [new_id for new_id in new_integer_ids],default=label)

        preprocess_dict = preprocess(label,inst_label)
        preprocess_dict['label'] = preprocess_dict['label']
        preprocess_dict['inst']  = preprocess_dict['inst']
        preprocess_dict['label'] = preprocess_dict['label'][0,:,:].unsqueeze(0).unsqueeze(0)
        if use_label_as_instance:
            preprocess_dict['inst'] = preprocess_dict['label']
        else:
            preprocess_dict['inst'] = preprocess_dict['inst'][0, :, :].unsqueeze(0).unsqueeze(0)
        st_time = time.time()
        if opt.no_instance:
            generated = model.inference(preprocess_dict['label'], torch.tensor(0), preprocess_dict['image'])
        else:
            generated = model.inference(preprocess_dict['label'], preprocess_dict['inst'], preprocess_dict['image'])
        print("Inference Time: " + str(time.time() - st_time))
